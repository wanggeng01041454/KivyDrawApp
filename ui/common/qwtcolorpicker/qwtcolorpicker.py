# -*- coding: utf-8 -*-
"""
复制自kivymd/uix/pickers/colorpicker/colorpicker.py

由于 kivymd 原始的 colorpicker 在手机横屏显示时，会出现一些问题，所以复制一份，修改一下
出现问题的原因在于： 手机横屏时， int(self.ids.gradient_widget.height - dp(100)) 的值常常为负数
"""
...
# __all__ 是一个特殊的 Python 变量，用于定义一个模块中哪些成员可以被 from module import * 语句导入。
...
class SelectAlphaChannelWidget(MDBoxLayout):
    ...
    # :class:`~kivymd.uix.colorpicker.QwtColorPicker` class.
...
class GradientTab(MDBoxLayout):
    ...
    def create_gradient_texture(
        ...
        gradient_widget_height = int(self.ids.gradient_widget.height)
        img = PilImage.new(
...
class QwtColorPicker(BaseDialog):
