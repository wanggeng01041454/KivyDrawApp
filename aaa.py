    ...
    def onLeftButtonDown(self, event):  # 点击鼠标左键后运行此函数
        ...
        if self.function == 7:  # 颜色填充
            for i in range(len(self.li)):
                if (self.X >= self.li[i][0] and self.X <= self.li[i][2]) and (
                        self.Y >= self.li[i][1] and self.Y <= self.li[i][3]):
                    if self.temp[i] == 'rect':
        ...
        if self.function == 4:
            self.canvas.create_text(event.x, event.y, font=("等线", int(self.size)), text=self.text,
                                    fill=self.foreColor)
            self.function = 1
    ...
    def onLeftButtonMove(self, event):  # 按下鼠标左键并移动后运行此函数
        ...
        if self.function == 1:  # 铅笔
            ...
            try:
                ...
                pass
            self.lastDraw = self.canvas.create_oval(self.X, self.Y, event.x, event.y, fill=self.backColor,
